...
$google_redirect_uri = base_url().'import/import-contacts-with-php.php';
...
if(isset($_SESSION['google_code'])) {
	...
	*/
	...
	echo $email_Id=$user;
/*
	$check=mysqli_query($con,"SELECT * FROM user_emails WHERE emai='".$email_Id."'");
	$exits_Email=mysqli_num_rows($con,$check);

	if($exits_Email>0){
		$up=mysqli_query($con,"UPDATE user_emails SET email='".$email_Id."' WHERE email='".$email_Id."'");
		if($up>0){
		?>
		...
		if($email_Id>0){
		$ins=mysqli_query($con,"INSERT INTO user_emails VALUES('$email_Id') WHERE email='".$email_Id."'");
		if($ins>0){
			?>
			<script type="text/javascript">
...
*/
			...
			$return[] = array (
				...
				'image' => @$image,
				'organization'=>@$contact['gd$organization'][0]['gd$orgName']['$t'],
				'organizationName'=>@$contact['gd$organization'][0]['gd$orgTitle']['$t'],
